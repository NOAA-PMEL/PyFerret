...
int read_int2_array( int f, short *iarray, int n )
{
   ...
   signed char *buffer;
   int nread;
   buffer = (signed char *) FerMem_Malloc( n * 2 );
   if (!buffer)  return 0;
...
int read_uint2_array( int f, unsigned short *iarray, int n )
{
   ...
   int nread;
   buffer = (unsigned char *) FerMem_Malloc( n * 2 );
   if (!buffer)  return 0;
   nread = read( f, buffer, n*2 );
   if (nread<=0)  return 0;
   nread /= 2;
...
int read_int4_array( int f, int *iarray, int n )
{
   ...
   buffer = (int *) FerMem_Malloc( (n+1)*4 );
   if (!buffer)
      return 0;
...
int read_float4_array( int f, float *x, int n )
{
   ...
   buffer = (long *) FerMem_Malloc( (n+1) * 4 );
   if (!buffer) return 0;
...
int write_uint2_array( int f, const unsigned short *iarray, int n )
{
   ...
   int i, nwritten;
   unsigned char *buffer;
   buffer = (unsigned char *) FerMem_Malloc( 2*n );
   if (!buffer)  return 0;
   for (i=0;i<n;i++) {
...
int write_int4_array( int f, const int *i, int n )
{
   ...
   b = buf = (char *) FerMem_Malloc( n*4 + 8 );
   if (!b)
      return 0;
   ptr = (char *) i;
   for (j=0;j<n;j++) {
...
int write_float4_array( int f, const float *x, int n )
{
   ...
   int nwritten;
   long *buffer;
   buffer = (long *) FerMem_Malloc( n*4 + 8 );
   if (!buffer)
      return 0;
