...
def ferret_init(efid):
    ...
    part_influence[0] = False
    part_influence[1] = False
...
def ferret_compute(efid, result, result_bdf, inputs, input_bdfs):
    ...
    rect_center_lons = pyferret.get_axis_coordinates(efid, pyferret.ARG6, pyferret.X_AXIS)
    rect_center_lats = pyferret.get_axis_coordinates(efid, pyferret.ARG6, pyferret.Y_AXIS)
    ...
    if not numpy.allclose(lo[1:], hi[:-1]):
        raise ValueError("Unexpected ARG6 Y_AXIS box limit values returned from pyferret.get_axis_box_limits")
