...
def ferret_compute(id, result, resbdf, inputs, inpbdfs):
    """
    Assigns to result[:,0,0,0] the order statistic medians 
    for the probability distribution named in inputs[1] with 
    parameters given in inputs[2].  Assigns to result[:,1,0,0] 
    the ordered response data of the sample values given in 
    inputs[0].  Assigns to result[:3,2,0,0] the slope, intercept, 
    and correlation coefficient of the line fitted to a plot of 
    result[:,1,0,0] against result[:,0,0,0].  Undefined values 
    ...
    result[badmask,0,0,0] = resbdf
    result[goodmask,0,0,0] = ppdata[0][0]
    result[badmask,1,0,0] = resbdf
    result[goodmask,1,0,0] = ppdata[0][1]
    result[3:,2,0,0] = resbdf
    result[:3,2,0,0] = ppdata[1]
...
if __name__ == "__main__":
    ...
    pfname = "norm"
    pfparams = numpy.array([0.0, 1.0], dtype=numpy.float32)
    inpbdfs = numpy.array([inpundefval, 0.0, 0.0], dtype=numpy.float32)
    resbdf = numpy.array([outundefval], dtype=numpy.float32)
    inputarr = numpy.empty((1, ydim + 1, zdim + 1, 1), dtype=numpy.float32, order='F')
    expected = numpy.empty(((ydim + 1) * (zdim + 1), 3, 1, 1), dtype=numpy.float32, order='F')
    n = 0
    index = 0
    for j in xrange(ydim + 1):
        for k in xrange(zdim + 1):
            if (k == j) or (k == j+1) or (n >= size):
                inputarr[0, j, k, 0] = inpbdfs[0]
                expected[index, 0, 0, 0] = resbdf[0]
                expected[index, 1, 0, 0] = resbdf[0]
            else:
                inputarr[0, j, k, 0] = sample[n]
                expected[index, 0, 0, 0] = osmeds[n]
                expected[index, 1, 0, 0] = ordata[n]
                n += 1
            index += 1
    if n < size:
        raise ValueError("Unexpected result: not all values assigned")
    fitdata = scipy.stats.linregress(osmeds, ordata)
    expected[:3,2,0,0] = fitdata[:3]
    expected[3:,2,0,0] = resbdf[0]
    result = -888888.0 * numpy.ones(((ydim + 1) * (zdim + 1), 3, 1, 1), dtype=numpy.float32, order='F')
    ...
    if not numpy.allclose(result, expected):
        if not numpy.allclose(result[:,0,0,0], expected[:,0,0,0]):
            print "Expected[:,0,0,0] =\n%s" % str(expected[:,0,0,0])
            print "Result[:,0,0,0] =\n%s" % str(result[:,0,0,0])
        if not numpy.allclose(result[:,1,0,0], expected[:,1,0,0]):
            print "Expected[:,1,0,0] =\n%s" % str(expected[:,1,0,0])
            print "Result[:,1,0,0] =\n%s" % str(result[:,1,0,0])
        if not numpy.allclose(result[:3,2,0,0], expected[:3,2,0,0]):
            print "Expected[:3,2,0,0] =\n%s" % str(expected[:3,2,0,0])
            print "Result[:3,2,0,0] =\n%s" % str(result[:3,2,0,0])
